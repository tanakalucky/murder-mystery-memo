...
- [ ] @xyflow/react によるキャンバス実装
- [ ] ビューポート全画面のキャンバスレイアウト
...
現在はTodoアプリ（Convex + Clerk連携）が実装されているが、メモアプリへ完全に置き換える。以下は削除対象：
...
| ------------------------------ | ------------------------------------------ | --------- |
| Convex/Clerk を完全削除        | メモアプリにバックエンド・認証は不要       | — Pending |
| @xyflow/react でキャンバス実装 | ノードのドラッグ・配置が主要機能と一致     | — Pending |
| localStorage で永続化          | シンプル、バックエンド不要、個人利用に十分 | — Pending |
